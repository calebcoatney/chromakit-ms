...
def match_peaks(true_peaks, components, max_rt_distance=0.5):
    ...
    if n_true == 0 and n_fitted == 0:
        return [], [], []
    if n_true == 0:
        phantoms = [{'rt': c.retention_time, 'sigma': c.sigma,
                     'tau': c.tau, 'area': c.area} for c in components]
        return [], [], phantoms
    if n_fitted == 0:
    ...
    for i, j in zip(row_ind, col_ind):
        if cost[i, j] <= max_rt_distance:
            tp = true_peaks[i]
            comp = components[j]
            sigma_ratio = comp.sigma / tp['sigma'] if tp['sigma'] > 1e-8 else None
            tau_ratio = comp.tau / tp['tau'] if tp['tau'] > 1e-8 else None
    ...
    phantoms = [{'rt': components[j].retention_time,
                 'sigma': components[j].sigma,
                 'tau': components[j].tau,
