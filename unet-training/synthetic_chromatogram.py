...
class SyntheticChromatogramGenerator:
    ...
    def _place_peaks(self, t, num_peaks, merge_probability,
                     ...
                     sigma_range, tau_range, dynamic_range=None):
        """
        ...
        while i < len(centres):
            ...
            if dynamic_range is not None:
                log_lo = np.log10(amplitude_range[1] / dynamic_range)
                log_hi = np.log10(amplitude_range[1])
                amp = 10 ** self.rng.uniform(log_lo, log_hi)
            else:
    ...
    def generate(self, *,
        ...
        dynamic_range : float or None
            When set, amplitudes are sampled log-uniformly. The upper end
            of ``amplitude_range`` is the ceiling; dividing by
            ``dynamic_range`` gives the floor. E.g. (50, 500) with
            dynamic_range=100 → log-uniform over [5, 500]. None = uniform.
